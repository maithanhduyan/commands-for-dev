...
- whisper 23nguyentackiemtien.mp3 --model medium --language vi

- whisper 23nguyentackiemtien.mp3 --model turbo --language vi

- whisper 23nguyentackiemtien.mp3 --model turbo --language vi --output_format txt


```
(.venv) PS D:\Project\commands-for-dev\AI\whisper> whisper -h                                                  
...
options:
  ...
  --condition_on_previous_text CONDITION_ON_PREVIOUS_TEXT
                        ...
                        inconsistent across windows, but the model becomes less prone to getting stuck in a failure loop (default:       
                        True)
  ...
  --clip_timestamps CLIP_TIMESTAMPS
                        comma-separated list start,end,start,end,... timestamps (in seconds) of clips to process, where the last end     
                        timestamp defaults to the end of the file (default: 0)
  --hallucination_silence_threshold HALLUCINATION_SILENCE_THRESHOLD
                        (requires --word_timestamps True) skip silent periods longer than this threshold (in seconds) when a possible    
                        hallucination is detected (default: None)
