...
echo "Create the docker-compose.yml file"
...
#####################################################################################################
# Tạo thư mục postgresql
mkdir -p ./postgresql/conf

# Tạo file cấu hình postgres mẫu: 
...
echo "./postgresql/conf/postgres.conf file created successfully!"
...
echo "/nginx/conf/default.conf file created successfully!"

# Create the directory for Nginx html file
# Tạo thư mục ./nginx/html nếu chưa tồn tại
mkdir -p ./nginx/html

# Tạo tệp index.html với nội dung đã cho
...
# Tạo tệp 404.html với nội dung đã cho
cat <<EOF > ./nginx/html/404.html
<!DOCTYPE html>
<html>
  <head>
...
# Tạo tệp 403.html với nội dung đã cho
cat <<EOF > ./nginx/html/403.html
<!DOCTYPE html>
<html>
  <head>
    <title>Error</title>
...
# Tạo tệp 5x.html với nội dung đã cho
cat <<EOF > ./nginx/html/5x.html
<!DOCTYPE html>
<html>
  <head>
    <title>Error</title>
...
# Tạo SSL cho server Nginx
mkdir -p ./nginx/ssl
cat <<EOF > ./nginx/ssl/server-sample.crt
-----BEGIN CERTIFICATE-----
...
cat <<EOF > ./nginx/ssl/server-sample.key
-----BEGIN PRIVATE KEY-----
...
# Tạo file odoo config

cat <<EOF > ./odoo/conf/odoo.conf
[options]
